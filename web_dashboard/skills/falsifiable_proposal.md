...
- `mechanism`: why the market would pay for this (economic / behavioral reason), in prose
- `mechanism_category`: **exactly one** of the values below — this is the axis your claim is scored on, so pick the closest fit rather than inventing a label:
  `information_diffusion`, `liquidity_premium`, `sentiment_reversal`, `earnings_surprise_drift`, `catalyst_repricing`, `valuation_mean_reversion`, `informed_trading_signal`, `supply_demand_imbalance`, `sector_rotation`, `narrative_momentum`, `structural_neglect`, `risk_repricing`, `other`
