...
def _format_portfolio_snapshot_table(
    ...
    # Rename any space-containing columns to avoid attribute errors with itertuples
    if any(' ' in str(col) for col in positions_df.columns):
        positions_df = positions_df.rename(columns=lambda x: str(x).replace(' ', '_'))

    # Calculate total portfolio value for % Port (Optimization: using itertuples)
    def _get_market_value(r):
        v = getattr(r, 'market_value', None)
        return float(v) if v is not None else 0.0

    total_portfolio_value = sum(_get_market_value(row) for row in positions_df.itertuples(index=False))
    ...
    for row in positions_df.itertuples(index=False):
        # Handle properties safely, falling back appropriately
        symbol = getattr(row, 'symbol', None)
        if symbol is None:
            symbol = getattr(row, 'ticker', 'N/A')

        q1 = getattr(row, 'quantity', None)
        q2 = getattr(row, 'shares', None)
        quantity = float(q1) if q1 is not None else (float(q2) if q2 is not None else 0.0)

        c = getattr(row, 'currency', None)
        currency = c if c is not None else 'CAD'

        cb = getattr(row, 'cost_basis', None)
        cost_basis = float(cb) if cb is not None else 0.0

        mv = getattr(row, 'market_value', None)
        market_value = float(mv) if mv is not None else 0.0

        up = getattr(row, 'unrealized_pnl', None)
        pnl = float(up) if up is not None else 0.0

        upp = getattr(row, 'unrealized_pnl_pct', None)
        rp = getattr(row, 'return_pct', None)
        pnl_pct = float(upp) if upp is not None else (float(rp) if rp is not None else 0.0)

        cp = getattr(row, 'current_price', None)
        pr = getattr(row, 'price', None)
        current_price = float(cp) if cp is not None else (float(pr) if pr is not None else 0.0)
        
        # Get daily P&L from view (may be None/null)
        dp = getattr(row, 'daily_pnl', None)
        daily_pnl = float(dp) if dp is not None else 0.0

        dpp = getattr(row, 'daily_pnl_pct', None)
        daily_pnl_pct = float(dpp) if dpp is not None else 0.0
        
        # Get 5-day P&L from view
        fdp = getattr(row, 'five_day_pnl', None)
        five_day_pnl = float(fdp) if fdp is not None else 0.0

        fdpp = getattr(row, 'five_day_pnl_pct', None)
        five_day_pnl_pct = float(fdpp) if fdpp is not None else 0.0
        
        # Get company name (truncate to 25 chars, matching console app format)
        comp = getattr(row, 'company', None)
        company = comp if comp is not None else ''
        if company:
...
def format_price_volume_table(positions_df: pd.DataFrame) -> str:
    ...
    for row in positions_df.itertuples(index=False):
        ...
        ticker = sym if sym is not None else getattr(row, 'ticker', 'N/A')

        cp = getattr(row, 'current_price', None)
        current_price = float(cp) if cp is not None else 0.0

        yp = getattr(row, 'yesterday_price', None)
        yesterday_price = float(yp) if yp is not None else 0.0
...
def format_fundamentals_table(positions_df: pd.DataFrame) -> str:
   ...
   # First pass: Read from DB (securities join) and identify stale data
    for row in positions_df.itertuples(index=False):
        ...
        ticker = tk if tk is not None else getattr(row, 'symbol', 'N/A')
...
def format_trades(trades_df: pd.DataFrame, limit: int = 100) -> str:
    ...
    if any(' ' in str(col) for col in df.columns):
        df = df.rename(columns=lambda x: str(x).replace(' ', '_'))

    for row in df.itertuples(index=False):
        # Handle both 'timestamp' and 'date' columns from different data sources
        ts = getattr(row, 'timestamp', None)
        dt = getattr(row, 'date', None)
        timestamp = ts if ts is not None else (dt if dt is not None else '')

        sym = getattr(row, 'symbol', None)
        symbol = sym if sym is not None else getattr(row, 'ticker', 'N/A')
        
        # Extract action from reason field (shared classifier)
        rs = getattr(row, 'reason', None)
        reason = rs if rs is not None else ''
        action = infer_trade_action(reason, default='BUY')
        
        # Handle both 'quantity' and 'shares' columns
        q1 = getattr(row, 'quantity', None)
        q2 = getattr(row, 'shares', None)
        quantity = q1 if q1 is not None else (q2 if q2 is not None else 0)

        pr = getattr(row, 'price', None)
        price = pr if pr is not None else 0

        c = getattr(row, 'currency', None)
        currency = c if c is not None else 'CAD'

        # Calculate total_value if not present
        tv = getattr(row, 'total_value', None)
        total_value = tv
