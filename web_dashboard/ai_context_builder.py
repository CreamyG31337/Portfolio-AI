...
def _format_portfolio_snapshot_table(
    ...
    total_portfolio_value = sum(safe_float(getattr(row, 'market_value', 0)) for row in positions_df.itertuples(index=False))
    ...
    for row in positions_df.itertuples(index=False):
        symbol = getattr(row, 'symbol', getattr(row, 'ticker', 'N/A'))
        if pd.isna(symbol): symbol = 'N/A'
        quantity = safe_float(getattr(row, 'quantity', getattr(row, 'shares', 0)))
        currency = getattr(row, 'currency', 'CAD')
        if pd.isna(currency): currency = 'CAD'
        cost_basis = safe_float(getattr(row, 'cost_basis', 0))
        market_value = safe_float(getattr(row, 'market_value', 0))
        pnl = safe_float(getattr(row, 'unrealized_pnl', 0))
        pnl_pct = safe_float(getattr(row, 'unrealized_pnl_pct', getattr(row, 'return_pct', 0)))
        current_price = safe_float(getattr(row, 'current_price', getattr(row, 'price', 0)))
        
        # Get daily P&L from view (may be None/null)
        daily_pnl = safe_float(getattr(row, 'daily_pnl', 0))
        daily_pnl_pct = safe_float(getattr(row, 'daily_pnl_pct', 0))
        
        # Get 5-day P&L from view
        five_day_pnl = safe_float(getattr(row, 'five_day_pnl', 0))
        five_day_pnl_pct = safe_float(getattr(row, 'five_day_pnl_pct', 0))
...
def format_price_volume_table(positions_df: pd.DataFrame) -> str:
    ...
    for row in positions_df.itertuples(index=False):
        ticker = getattr(row, 'symbol', getattr(row, 'ticker', 'N/A'))
        if pd.isna(ticker): ticker = 'N/A'
        current_price = safe_float(getattr(row, 'current_price', 0))
        yesterday_price = safe_float(getattr(row, 'yesterday_price', 0))
...
def format_fundamentals_table(positions_df: pd.DataFrame) -> str:
   ...
   # First pass: Read from DB (securities join) and identify stale data
    for row in positions_df.itertuples(index=False):
        ticker = getattr(row, 'ticker', getattr(row, 'symbol', 'N/A'))
        if pd.isna(ticker): ticker = 'N/A'
        
        # Extract securities data
        securities = getattr(row, 'securities', None)
        if pd.isna(securities): securities = None
        sec_data = {}
        if securities:
...
def format_trades(trades_df: pd.DataFrame, limit: int = 100) -> str:
    ...
    for row in df.itertuples(index=False):
        ...
        timestamp = getattr(row, 'timestamp', getattr(row, 'date', ''))
        if pd.isna(timestamp): timestamp = ''
        symbol = getattr(row, 'symbol', getattr(row, 'ticker', 'N/A'))
        if pd.isna(symbol): symbol = 'N/A'
        ...
        quantity = getattr(row, 'quantity', getattr(row, 'shares', 0))
        ...
        if pd.isna(total_value): total_value = None
