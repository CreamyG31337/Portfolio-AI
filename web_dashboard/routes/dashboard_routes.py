    ...
    try:
        ...
        def calculate_display_amount(row, action):
            ...
            if action == 'SELL':
                ...
                return abs(float(getattr(row, 'amount', 0)))
            else:
                # For BUYs/DRIPs: show purchase amount
                shares = abs(float(getattr(row, 'shares', 0) or 0))
                price = float(getattr(row, 'price', 0) or 0)
                return shares * price
        
        data = []
        # Optimize: Use itertuples instead of iterrows for better performance
    ...
    try:
        ...
        def df_to_list(df, logo_map=None, company_map=None):
            ...
            for row in df.itertuples(index=False):
                ...
                company_name_val = getattr(row, 'company_name', None)

                # Compute company name fallback
                if company_name_val:
                    c_name = company_name_val
                elif company_map:
                    c_name = company_map.get(ticker, ticker)
                else:
                    c_name = ticker
                ...
                if hasattr(row, 'daily_pnl_pct'):
                    val = getattr(row, 'daily_pnl_pct')
                    item["daily_pnl_pct"] = float(val) if pd.notna(val) else None
                elif hasattr(row, 'return_pct'):
                    val = getattr(row, 'return_pct')
                    item["daily_pnl_pct"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'pnl_display'):
                    val = getattr(row, 'pnl_display')
                    item["daily_pnl"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'five_day_pnl_pct'):
                    val = getattr(row, 'five_day_pnl_pct')
                    item["five_day_pnl_pct"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'five_day_pnl_display'):
                    val = getattr(row, 'five_day_pnl_display')
                    item["five_day_pnl"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'return_pct') and 'daily_pnl_pct' in columns:
                    val = getattr(row, 'return_pct')
                    item["total_return_pct"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'total_pnl_display'):
                    val = getattr(row, 'total_pnl_display')
                    item["total_pnl"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'current_price'):
                    val = getattr(row, 'current_price')
                    item["current_price"] = float(val) if pd.notna(val) else None

                if hasattr(row, 'market_value'):
                    val = getattr(row, 'market_value')
