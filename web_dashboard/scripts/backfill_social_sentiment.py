...
Usage:
    ...
    python scripts/backfill_social_sentiment.py extract --days 14 [--platform stocktwits] [--apply]
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    checks = [
        ...
        (
            "  ...not yet in a session",
            "SELECT COUNT(*) n FROM social_posts WHERE session_id IS NULL",
        ),
        (
            "  ...epoch-dated (should be 0)",
            "SELECT COUNT(*) n FROM social_posts WHERE posted_at < '2000-01-01'",
        ),
...
def cmd_retire_orphans(args: argparse.Namespace) -> int:
    ...
    A session with no social_posts rows has no content and never will. Left
    pending it sits at the head of the oldest-first queue and starves
...
def cmd_sessions(args: argparse.Namespace) -> int:
    ...
    for _ in range(args.max_passes):
        ...
        if result["sessions_created"] == 0 and result["posts_assigned"] == 0:
            break
        ...
        logger.warning("Hit --max-passes (%s); re-run to continue", args.max_passes)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("retire-orphans", help="retire pending sessions with no posts")
    ...
    p = sub.add_parser("sessions", help="group posts into per-ticker-per-day sessions")
