...
## $(date +%Y-%m-%d) - Pandas Row Iteration Bottleneck
**Learning:** `df.iterrows()` iterates row by row by returning Series objects, which involves a ton of overhead. It's often replaced by `df.itertuples(index=False)` in backend routes for significant performance gains, as namedtuples are much faster to construct. When dealing with spaces in column names, `df.rename(columns={'A B': 'A_B'})` is necessary because spaces cannot be used as namedtuple attributes.
**Action:** Replace `df.iterrows()` with `df.itertuples(index=False)` whenever possible. Safely access data using `getattr(row, 'column_name')` and `hasattr(row, 'column_name')`.
