...
## 2026-04-16 - O(N) DataFrame Iteration Overheads
**Learning:** Standard `df.iterrows()` iteration over Pandas Dataframes incurs a massive performance penalty due to row-by-row Series conversion. Found instances in `jobs_portfolio.py`, `app.py`, and `rebuild_from_date.py` where iterating heavily nested rows caused extreme CPU overhead (>50x slower).
**Action:** For all future iterations over Pandas DataFrames in performance-critical areas, replace `.iterrows()` with `.itertuples(index=False, name=None)`. Access values via indices pre-calculated before the loop using `df.columns.get_loc('ColName')` for O(1) lookups and C-level speed.
