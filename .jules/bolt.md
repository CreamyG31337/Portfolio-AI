...
## 2024-05-15 - Vectorizing and avoiding `iterrows()`
**Learning:** `iterrows()` is slow and instantiates a Series object per row in Pandas, which adds massive interpreter overhead in loops.
**Action:** Replace it with vectorized operations like `df['Date'].dt.normalize()` when possible. When loop processing is mandatory, use `to_dict('records')` upfront to convert the dataframe into dictionaries, achieving O(1) bulk conversion and significantly faster iteration.
