...
## $(date +%Y-%m-%d) - Pandas iterrows() Performance Anti-Pattern in Serialization
**Learning:** Found significant performance overhead in `web_dashboard/ai_context_builder.py` where `df.iterrows()` was used to serialize thousands of portfolio positions and trade records into AI prompt context. `iterrows()` creates a new pandas Series for every single row, resulting in massive overhead (often 10-50x slower) for simple read operations. This becomes a severe bottleneck as the database sizes grow.
**Action:** When iterating through pandas DataFrames strictly for reading or serializing values (especially inside large formatting loops), ALWAYS replace `df.iterrows()` with `df.itertuples(index=False)`. Replace dict-style `.get('column', default)` access with `getattr(row, 'column', default)`, ensuring you handle `None` explicitely (e.g., `val if val is not None else default`).
