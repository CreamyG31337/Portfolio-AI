...
## 2026-07-13 - Boolean Filtering with List Comprehensions in Pandas
**Learning:** When replacing `.apply()` with list comprehensions for boolean filtering in Pandas (e.g., `df = df[[func(x) for x in df['col'].tolist()]]`), it creates a dangerous bug. If the dataframe is empty, the list comprehension evaluates to `[]`. Pandas treats `df[[]]` as a column selection request for zero columns, which strips the dataframe of all columns, causing immediate `KeyError` crashes on downstream operations.
**Action:** When using list comprehensions for boolean masks in Pandas, always wrap the result in a numpy array with an explicit boolean dtype: `df = df[np.array([func(x) for x in df['col'].tolist()], dtype=bool)]`. This forces Pandas to interpret it as a boolean row mask, correctly returning an empty DataFrame with intact columns when the input list is empty.
