...
## 2026-06-03 - Vectorization over iterrows() with datetime operations
**Learning:** Found a performance bottleneck in `trading_script.py` where `.iterrows()` was used to loop over a merged Pandas DataFrame to add 13 hours to a `Date` column based on whether the date was a weekday or weekend. However, the logic added exactly 13 hours in *both* cases, making the conditional entirely redundant. Iterating row-by-row in Pandas using `.iterrows()` is highly inefficient (O(N) with significant per-row Series instantiation overhead).
**Action:** When performing uniform transformations on columns (like datetime arithmetic), replace `.iterrows()` loops with fully vectorized operations (e.g., `df['Date'] = pd.to_datetime(df['Date']).dt.normalize() + pd.Timedelta(hours=13)`). This shifts execution to optimized C-level code, drastically improving performance, especially over long date ranges. Always check if conditional logic inside an iteration loop can be simplified or eliminated to unlock vectorization.
