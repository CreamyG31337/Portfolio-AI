...
## 2026-03-28 - Optimizing iterrows on large datasets
**Learning:** Found that `DataFrame.iterrows()` causes severe overhead due to generating Pandas Series objects for every row, which can stall processing for larger tables like `portfolio_positions` on the dashboard. Also, trying to parse multi-word columns using `namedtuples` creates invalid property keys in `itertuples`.
**Action:** Replaced `.iterrows()` with `.itertuples(index=False)` combined with `getattr()` for fallback values. For columns containing spaces (e.g., CSV imports), used `.itertuples(index=False, name=None)` to extract standard positional tuples while first slicing the DataFrame to enforce the expected column order.
