...
**Action:** Always wrap data models representing collections with internal dictionary caches to provide $O(1)$ access. Added `_positions_by_ticker` via `__post_init__` to `PortfolioSnapshot`, keeping it cleanly synced during `add_position` and `remove_position` mutations while offering an explicit fallback for deserialization pipelines that bypass the constructor.## 2026-03-05 - Pandas DataFrame Iteration Performance
**Learning:** Found multiple instances of `iterrows()` used for iterating over Pandas DataFrames in `web_dashboard/scheduler/jobs_etf_watchtower.py` and `web_dashboard/supabase_client.py`. `iterrows()` is notoriously slow because it type-checks and constructs a Series object for every row.
**Action:** Replace `iterrows()` with `itertuples(index=False, name=None)` which yields standard Python tuples and operates 10-100x faster, or use vectorized operations (e.g. `df['col'].abs().round(1).tolist()`) whenever possible. Also, when using `itertuples(index=False, name=None)`, pre-compute column indices using `df.columns.get_loc('col')` before the loop for fast index-based tuple access.
