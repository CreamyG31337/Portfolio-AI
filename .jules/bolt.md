...
## 2026-03-24 - Cross-User Cache Leak Anti-Pattern
**Learning:** Found that applying `@cache_data` decorators directly to Flask data fetching functions (like `get_current_positions_flask`, `get_trade_log_flask`, and `get_cash_balances_flask`) without passing `user_id` as an explicit argument caused cross-user data leakage. Because `user_id` was retrieved *inside* the decorated function, it was missing from the generated cache key, meaning one user's cached data could be served to another user.
**Action:** When caching user-specific data in Flask, ALWAYS extract `user_id` in the public wrapper function and pass it as an explicit argument to an internal `_cached` helper function (e.g., `def _get_data_cached(..., user_id)`). This ensures the cache key generator includes the `user_id`, properly scoping the cache per user.
