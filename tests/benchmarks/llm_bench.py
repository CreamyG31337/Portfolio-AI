...
- Live Ollama (``OLLAMA_ENABLED`` not false) and models available on resolved hosts.
  For ``qwen3.8:27b-mtp-q4_K_M``, set ``OLLAMA_BASE_URL_2`` to the Ollama base URL of the host that has that model installed;
...
Optional context sweep (same article sample, one model)::
    ...
    set OLLAMA_BENCH_CTX_MODEL=qwen3.8:27b-mtp-q4_K_M
...
def test_bench_article_summary_and_crowd_json() -> None:
    ...
    models_env = os.getenv("OLLAMA_BENCH_MODELS", "granite4.1:8b,qwen3.8:27b-mtp-q4_K_M")
