...
def test_cash_dividend_credits_cash_balances() -> None:
    ...
    _credit_cash_dividends_batch(client, {("RRSP Lance Webull", "USD"): Decimal("10.72")})

    upsert_call = cash_table.upsert.call_args
    payload = upsert_call[0][0]
    assert payload[0]["fund"] == "RRSP Lance Webull"
    assert payload[0]["currency"] == "USD"
    assert payload[0]["amount"] == 2350.30
...
def test_cash_dividend_handles_missing_cash_row() -> None:
    ...
    _credit_cash_dividends_batch(client, {("RRSP Lance Webull", "CAD"): Decimal("48.50")})

    payload = cash_table.upsert.call_args[0][0]
    assert payload[0]["currency"] == "CAD"
    assert payload[0]["amount"] == 48.50


from scheduler.jobs_dividends import prepare_drip_transaction
...
def test_cash_credit_failure_does_not_abort_dividend_log(
    ...
    assert result["success"] is True
    assert result["is_drip"] is False
