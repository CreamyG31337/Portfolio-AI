...
| **A4** | **Tool catalog v2** (partly ✅ via **A8** 2026-07-24) | **Shipped in A8:** portfolio performance, trade history, price history, and a widened `search_web` window. **Realized P&L in `get_trade_history`** ✅ (2026-07-25) — no recompute needed: FIFO cost-basis / partial-sell / native-currency handling is already done upstream and persisted to `trade_log.pnl`/`cost_basis`, so the tool just surfaces it (per-currency, never summed across CAD+USD). **Still open:** lean tools for thesis attention, confluence / liquidity / earnings calendar, Ideas triage peek, and a track-record / source-ROI slice — so open-ended questions don’t invent Learn answers. |
