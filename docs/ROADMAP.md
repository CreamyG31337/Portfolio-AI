...
> **Correction (2026-07-27).** This section used to claim "accept/dismiss clicks become labeled
> data for the article relevance scorer." **Retract that.** At one user's click volume the label
> count is orders of magnitude short of training a text relevance model; "Dismiss" conflates
> *junk* / *real idea, bad stock* / *already knew* / *no time*; and ranking-then-labelling-the-top
> is a bandit with no exploration, so it would confirm whatever it started believing.
>
> Ideas is a **universe-selection tool, not a measurement tool.** Accept's value is as a
> *routing action* — it admits a ticker into the nightly analysis universe, where
> `stance_history` → `stance_outcomes` already produces a real, outcome-based signal with no
> clicks required. "Which alpha sources produce ideas worth having?" is therefore answerable via
> `research_articles → stance_history.metadata->'evidence'->'article_ids' → stance_outcomes`,
> a join whose plumbing G1 already built. Empty `idea_triage` blocks **no** measurement.
...
| **Quality harness** | `web_dashboard/scripts/quality_test_ai_assistant_chat.py` → `verification/ai_assistant_quality_latest.json`. Offline probes still fail for Flask-bound tools (`get_holdings_snapshot` / `get_trade_history` → “Working outside of request context”); see wishlist **A10**. |
| **A11 · Chat session persistence** (2026-07-25) | Server-side transcript per `(user_id, fund)` in Supabase `ai_assistant_chats`. Restore on load / fund change; append after each completed reply; Clear Chat wipes DB + WebAI disk session; pagehide saves an interrupted stub if mid-stream. |
...
| **A3 · Single ranking source** (2026-07-24) | `advise_service.rank_candidate_pack` is the one builder both the chat pulse (`ai_intelligence_pulse._fetch_candidates`) and Today (`today_briefing_service.build_today_briefing`) call: `build_advise_recommendations` → shared held-gated **signal fallback** when advise is empty → A2 tension annotate + demote. Today `advise_pack` now **inherits** the signal fallback + A2 `tension` badge (+ `advise_source`), so when the queue/Insights are empty Today shows the same watchlist candidates as chat instead of "no items". Chat pulse stays held-safe: it passes **no** Insights/confluence Learn inputs (a bearish thesis / BUY→RISK flip on a non-held name would manufacture an unheld SELL/RISK) — held-gated Learn reweight in chat is deferred to **A7**. |
| **A8 · History & investigation tools** (2026-07-24) | Four read-only tools so the chat can answer past-performance / "what moved this" questions instead of being capped at the auto-injected 365-day summary (`ai_assistant_tools.py`): **`get_portfolio_performance`** (windowed total return / peak / max drawdown / downsampled equity curve; `window='all'` = since inception — wraps `calculate_portfolio_value_over_time_flask`), **`get_trade_history`** (past executed trades filtered by ticker/action/since + buy/sell summary — wraps `get_trade_log_flask`; sell rows now carry `realized_pnl` and the summary reports `realized_pnl_by_currency`, surfaced from the FIFO-computed `trade_log.pnl`/`cost_basis` stored at sell time — added 2026-07-25), **`get_price_history`** (a ticker's daily closes: high/low, % change, the 5 biggest single-day moves with dates + downsampled curve — wraps `MarketDataFetcher.fetch_price_data`), and **`search_web` widened** with a `time_range` bucket (day/week/month/year, default week) so older events are reachable. All thin wrappers (no new collectors, no auto-trading); pure read/report so A2/A3 held-safety is untouched. Token control: each history payload is capped to ≤12 curve points + ≤5 moves server-side. Question matrix gains families 11 (`portfolio_performance`) + 12 (`event_investigation`, price_history→search_web). Partially delivers wishlist **A4**. |
| **A9 · Decide/Learn read tools** (2026-07-25) | Five more read-only wrappers finishing wishlist **A4** so open-ended questions consume existing scored artifacts instead of inventing Learn answers (`ai_assistant_tools.py`): **`get_track_record`** (Learn-layer hit rate + avg excess-return vs benchmark sliced by stance source / verdict / evidence domain = source-ROI, plus best/worst calls — wraps `build_track_record_summary`; ranks sources by sample size, exposes `hit_rate_pct`), **`get_theses_attention`** (human thesis threads flagged due/stale/weak or LLM `TENSION`/`STALE_THESIS` — wraps `list_theses_attention`; advisory, never auto-trade), **`get_confluence`** (recent confluence events = independent signal families aligning on a ticker, score + bullish/risk direction — wraps `fetch_recent_confluence_events`), **`get_ideas_triage`** (untriaged Alpha Research / Opportunity Discovery from the last 14d, highest relevance first — wraps `fetch_alpha_ideas`), and **`get_earnings_calendar`** (next scheduled earnings per ticker via on-demand yfinance `.calendar`, no DB writes / no scheduler — compose with `get_holdings_snapshot` for "holdings reporting soon?"). Question matrix gains families 13–17. Prompt catalog + two compose rules added so the model routes to them. Unit tests in `tests/test_ai_assistant_wishlist_tools.py` (services mocked); live probe confirmed all five return real data. All pure read/report — A2/A3 held-safety untouched. |
...
| **A2** | ✅ **shipped 2026-07-24** — Contradiction-aware pulse / candidates | Quality runs showed signal `SELL` + analysis `stance: BUY` on the same ticker (e.g. CLS.TO, MU). Now flags/demotes TENSION and held-gates non-held SELL. Today `advise_pack` UI surfaces `dual_tension` **and** the A2 `tension` badge as of A3 (2026-07-24). Still open: a softer "unconfirmed" flag for WATCH-vs-directional-stance divergence (intentionally out of scope to avoid noise). |
| **A3** | ✅ **shipped 2026-07-24** — Single ranking source with Today Advise | `advise_service.rank_candidate_pack` now the one builder for chat pulse + Today `advise_pack` (advise → held-gated signal fallback → A2). Today inherited fallback + `tension` badge + `advise_source`. Follow-up lives in **A7** (held-gated Learn reweight in chat) and optionally routing the `list_entry_candidates` tool through the same wrapper. |
| **A4** | ✅ **shipped** (**A8** 2026-07-24 + **A9** 2026-07-25) — Tool catalog v2 | **A8:** portfolio performance, trade history, price history, widened `search_web`; **realized P&L in `get_trade_history`** ✅ (2026-07-25, surfaced from persisted FIFO `trade_log.pnl`/`cost_basis`, per-currency). **A9 (2026-07-25):** `get_track_record` (source-ROI / hit rate), `get_theses_attention`, `get_confluence`, `get_ideas_triage`, `get_earnings_calendar` (on-demand yfinance). All thin read wrappers over existing artifacts — open-ended questions now consume scored Learn/Decide outputs instead of inventing them. Liquidity slice intentionally deferred (no existing scored artifact to wrap). |
...
| **A7** | **Learn reweight in chat candidates** | Mirror Advise v1: multiply candidate scores by track-record hit rates when n is large enough, so chat discovery inherits the Learn→Decide arrow. Seam is ready — pass `track_record` (and held-safe Insights/confluence) into `rank_candidate_pack` from the chat pulse; A3 deliberately withheld those from chat to avoid manufacturing unheld SELL/RISK, so this must held-gate thesis/confluence-derived SELL/RISK before enabling. |
| **A10** | **Quality harness Flask app-context shim** | Offline `quality_test_ai_assistant_chat.py` tool probes hit “Working outside of request context” for `get_holdings_snapshot` / `get_trade_history` (and thus can’t exercise earnings→holdings compose or any holdings/trade question). Small fix: push a Flask `app.app_context()` (and whatever fund/session stubs those wrappers need) in the harness so the full matrix can run without a live request. Does not change production tool behavior. |
