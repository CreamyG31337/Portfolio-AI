...
| **A4** | **Tool catalog v2** (partly ✅ via **A8** 2026-07-24) | **Shipped in A8:** portfolio performance, trade history, price history, and a widened `search_web` window. **Still open:** lean tools for thesis attention, confluence / liquidity / earnings calendar, Ideas triage peek, a track-record / source-ROI slice, and **realized P&L per round-trip** in `get_trade_history` (deferred — cost-basis subtlety: FIFO vs average, partial sells, multi-currency) — so open-ended questions don’t invent Learn answers. |
