...
## 2026-02-01 - Silent Broken Interactions
**Learning:** Broken interactive elements can hide in plain sight when visual feedback is missing. The 'Refresh Jobs' button had a mismatched ID in JS vs HTML, meaning it was completely non-functional, but users wouldn't know because there was no visual feedback (like a spinner) to indicate "nothing is happening".
**Action:** When adding visual feedback states (like loading spinners), verify the event listener attachment first. The act of adding the interaction often reveals underlying broken logic. Always ensure ID consistency between templates and scripts.
