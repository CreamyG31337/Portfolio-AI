...
def create_continuous_timeline(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Set to market close time: 1:00 PM PST (13:00) for all days.
    # This keeps lines flat over weekends and prevents diagonal lines.
...
def download_benchmark(benchmark_name: str, start_date: pd.Timestamp, end_date: pd.Timestamp, portfolio_dates: pd.Series) -> pd.DataFrame:
    ...
    try:
        ...
        # ⚡ Bolt: Vectorized Pandas Operations
        # Replaced O(N) df.iterrows() with O(1) vectorized datetime operations.
        # Set to market close time: 1:00 PM PST (13:00) to match portfolio timing for all days.
        merged['Date'] = merged['Date'].dt.normalize() + pd.Timedelta(hours=13)
