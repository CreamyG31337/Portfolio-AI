    ...
    try:
        ...
        for key, config in benchmarks.items():
            try:
                ...
                # OPTIMIZATION: Replaced O(N) .iterrows() loop with fully vectorized O(1) datetime operations
                merged['Date'] = pd.to_datetime(merged['Date']).dt.normalize() + pd.Timedelta(hours=13)
