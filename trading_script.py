    ...
    try:
        ...
        for key, config in benchmarks.items():
            try:
                ...
                # Apply market close timing (1:00 PM PST) for consistency using vectorized operations
                merged['Date'] = pd.to_datetime(merged['Date']).dt.normalize() + timedelta(hours=13)
